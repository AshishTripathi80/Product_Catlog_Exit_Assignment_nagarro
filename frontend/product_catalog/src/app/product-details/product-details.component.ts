    ...
    this.productService.checkServiceability(productId, pincode).subscribe(
      (response) => {
        ...
        if (response.isServiceable) {
          this.serviceabilityMessage = `Serviceability: Serviceable. Delivery Time: ${response.deliveryTime} Days.`;
        } else {
          this.serviceabilityMessage = 'Serviceability: Not Serviceable.';
        }
      },
      (error) => {
        console.error('Error checking serviceability:', error);
        this.serviceabilityMessage =
          'Error checking serviceability. Please try again later.';
      }
    );
  }
}
